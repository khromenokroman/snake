...
Food::Food(uint64_t cell_size, uint64_t cell_count, uint64_t offset)
    : m_position{0, 0}, m_cell_size{cell_size}, m_cell_count{cell_count}, m_offset{offset} {
    Image food_image = LoadImage("/usr/share/snake/bitmaps/food.png");
...
void Food::draw() const { DrawTexture(m_texture, m_offset + m_position.x * m_cell_size, m_offset + m_position.y * m_cell_size, WHITE); }
